...
import IntendedEffectsPanel from '../intended-effects/IntendedEffectsPanel'
import SelectedRecord from '../shared/SelectedRecord'

const NAME = 'IntendedEffectsSection'
...
export class IntendedEffectsSection extends Component {
      ...
      <div className='row'>
        {((!store.isEmpty) || (store.isEmpty && !store.isSelected)) &&
          <div className={store.isSelected ? halfGrid : fullGrid}>
            <IntendedEffectsPanel title={this.props.title} store={store} actionsDisabled={this.props.actionsDisabled} />
          </div>
        }
        {store.isSelected &&
...
IntendedEffectsSection.displayName = NAME
IntendedEffectsSection.propTypes = propTypes
IntendedEffectsSection.defaultProps = defaultProps

export default withRouter(observer(['stores'], IntendedEffectsSection))
