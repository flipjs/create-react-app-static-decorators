...
import DomainNamesPanel from '../domain-names/DomainNamesPanel'
import SelectedRecord from '../shared/SelectedRecord'

const NAME = 'DomainNamesSection'
...
export class DomainNamesSection extends Component {
      ...
      <div className='row'>
        {((!store.isEmpty) || (store.isEmpty && !store.isSelected)) &&
          <div className={store.isSelected ? halfGrid : fullGrid}>
            <DomainNamesPanel title={this.props.title} store={store} actionsDisabled={this.props.actionsDisabled} />
          </div>
        }
        {store.isSelected &&
...
DomainNamesSection.displayName = NAME
DomainNamesSection.propTypes = propTypes
DomainNamesSection.defaultProps = defaultProps

export default withRouter(observer(['stores'], DomainNamesSection))
