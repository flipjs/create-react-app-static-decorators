...
export class IndicatorsSection extends Component {
      ...
      <div className='row'>
        {((!store.isEmpty) || (store.isEmpty && !store.isSelected)) &&
          <div className={store.isSelected ? halfGrid : fullGrid}>
            <IndicatorsPanel title={this.props.title} store={store} actionsDisabled={this.props.actionsDisabled} />
          </div>
        }
        {store.isSelected &&
...
IndicatorsSection.displayName = NAME
IndicatorsSection.propTypes = propTypes
IndicatorsSection.defaultProps = defaultProps

export default withRouter(observer(['stores'], IndicatorsSection))
