...
export class IndicatorForm extends Component {
  addNewRecord (record) {
    const {indicators} = this.props.stores
    return indicators.save(record)
  }

  changeRouteToIndicatorsView () {
    this.props.router.push('/indicators')
  }

  handleDismiss = () => {
    this.changeRouteToIndicatorsView()
  }
  ...
  handleSubmit = () => {
    ...
    promise
      .then(() => {
        const message = 'Indicator has been saved successfully.'
  ...
  showNotification (type, message) {
    this.props.stores.ui.setNotification({
      title: this.props.stores.indicators.displayName,
      level: type,
      message
    })
  }
  ...
  updateExistingRecord (record) {
    const {indicators} = this.props.stores
    const existingRecord = indicators.selected
    return indicators.update(existingRecord, record)
  }
  ...
  render () {
    ...
    const {inputs: icons} = ICONS.domains.indicators

    return (
      <Panel id={NAME} theme={this.props.theme}>
        <PanelHeader icon={ICONS.indicators} title={title}>
          <PanelActions>
            <PanelAction
...
IndicatorForm.displayName = NAME
IndicatorForm.propTypes = propTypes
IndicatorForm.defaultProps = defaultProps

export default withRouter(observer(['stores'], IndicatorForm))
