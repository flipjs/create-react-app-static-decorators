...
import AddressesPanel from '../addresses/AddressesPanel'
import SelectedRecord from '../shared/SelectedRecord'

const NAME = 'AddressesSection'
...
export class AddressesSection extends Component {
      ...
      <div className='row'>
        {((!store.isEmpty) || (store.isEmpty && !store.isSelected)) &&
          <div className={store.isSelected ? halfGrid : fullGrid}>
            <AddressesPanel title={this.props.title} store={store} actionsDisabled={this.props.actionsDisabled} />
          </div>
        }
        {store.isSelected &&
...
AddressesSection.displayName = NAME
AddressesSection.propTypes = propTypes
AddressesSection.defaultProps = defaultProps

export default withRouter(observer(['stores'], AddressesSection))
