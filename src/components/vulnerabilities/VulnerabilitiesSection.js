...
import VulnerabilitiesPanel from '../vulnerabilities/VulnerabilitiesPanel'
import SelectedRecord from '../shared/SelectedRecord'

const NAME = 'VulnerabilitiesSection'
...
export class VulnerabilitiesSection extends Component {
      ...
      <div className='row'>
        {((!store.isEmpty) || (store.isEmpty && !store.isSelected)) &&
          <div className={store.isSelected ? halfGrid : fullGrid}>
            <VulnerabilitiesPanel title={this.props.title} store={store} actionsDisabled={this.props.actionsDisabled} />
          </div>
        }
        {store.isSelected &&
...
VulnerabilitiesSection.displayName = NAME
VulnerabilitiesSection.propTypes = propTypes
VulnerabilitiesSection.defaultProps = defaultProps

export default withRouter(observer(['stores'], VulnerabilitiesSection))
