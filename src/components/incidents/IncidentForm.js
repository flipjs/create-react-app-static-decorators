...
export class IncidentForm extends Component {
  addNewRecord (record) {
    const {incidents} = this.props.stores
    return incidents.save(record)
  }

  changeRouteToIncidentsView () {
    this.props.router.push('/incidents')
  }

  handleDismiss = () => {
    this.changeRouteToIncidentsView()
  }
  ...
  handleSubmit = () => {
    ...
    promise
      .then(() => {
        const message = 'Incident has been saved successfully.'
  ...
  showNotification (type, message) {
    this.props.stores.ui.setNotification({
      title: this.props.stores.incidents.displayName,
      level: type,
      message
    })
  }
  ...
  updateExistingRecord (record) {
    const {incidents} = this.props.stores
    const existingRecord = incidents.selected
    return incidents.update(existingRecord, record)
  }
  ...
  render () {
    ...
    const {inputs: icons} = ICONS.domains.incidents

    return (
      <Panel id={NAME} theme={this.props.theme}>
        <PanelHeader icon={ICONS.incidents} title={title}>
          <PanelActions>
            <PanelAction
...
IncidentForm.displayName = NAME
IncidentForm.propTypes = propTypes
IncidentForm.defaultProps = defaultProps

export default withRouter(observer(['stores'], IncidentForm))
