...
export class IncidentsSection extends Component {
      ...
      <div className='row'>
        {((!store.isEmpty) || (store.isEmpty && !store.isSelected)) &&
          <div className={store.isSelected ? halfGrid : fullGrid}>
            <IncidentsPanel title={this.props.title} store={store} actionsDisabled={this.props.actionsDisabled} />
          </div>
        }
        {store.isSelected &&
...
IncidentsSection.displayName = NAME
IncidentsSection.propTypes = propTypes
IncidentsSection.defaultProps = defaultProps

export default withRouter(observer(['stores'], IncidentsSection))
