...
import AdvisoriesPanel from '../advisories/AdvisoriesPanel'
import SelectedRecord from '../shared/SelectedRecord'

const NAME = 'AdvisoriesSection'
...
export class AdvisoriesSection extends Component {
      ...
      <div className='row'>
        {((!store.isEmpty) || (store.isEmpty && !store.isSelected)) &&
          <div className={store.isSelected ? halfGrid : fullGrid}>
            <AdvisoriesPanel title={this.props.title} store={store} actionsDisabled={this.props.actionsDisabled} />
          </div>
        }
        {store.isSelected &&
...
AdvisoriesSection.displayName = NAME
AdvisoriesSection.propTypes = propTypes
AdvisoriesSection.defaultProps = defaultProps

export default withRouter(observer(['stores'], AdvisoriesSection))
